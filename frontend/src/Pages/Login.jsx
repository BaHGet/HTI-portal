...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:3000/api/login/auth", loginData); // POST LOGIN DATA (ID, PASSWORD)
      const token = res.data.token; // GET the token from the response
      localStorage.setItem("token", token); // save the token in the internal storage
